...
@section('seo_title', $category->name . ' - ' . \App\Models\Setting::get('store_name', config('app.name')))
@section('seo_description', 'Jelajahi produk kategori ' . $category->name . ' di toko kami.')
@section('seo_url', route('shop.category', $category->slug))
...
<div class="shop-page flash-sale-page">
    <div class="flash-sale-page__header">
        <div>
            <h1>{{ $category->name }}</h1>
            <p>Produk kategori {{ $category->name }}</p>
        </div>
    </div>
    ...
    @else
        <div class="empty-state">
            ...
            <div>
                ...
                <p class="empty-state__text">Belum ada produk di kategori ini</p>
            </div>
        </div>
    @endif
</div>
