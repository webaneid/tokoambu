<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-900">Edit Kategori Produk</h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('product-categories.update', $productCategory) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $productCategory->name) }}" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" required>
                            @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                            <textarea name="description" rows="3" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">{{ old('description', $productCategory->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <input type="hidden" name="custom_fields" id="customFieldsData" value="{{ old('custom_fields', json_encode($productCategory->custom_fields ?? [])) }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const existingFields = {!! json_encode($productCategory->custom_fields ?? []) !!};
