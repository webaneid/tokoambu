...
@section('title', 'Semua Produk - ' . config('app.name'))
@section('seo_title', 'Semua Produk - ' . \App\Models\Setting::get('store_name', config('app.name')))
@section('seo_description', 'Jelajahi seluruh produk yang tersedia di toko kami.')
@section('seo_url', route('shop.all'))
...
<div class="shop-page">
    <div class="flash-sale-page__header">
        <div>
            <h1>Semua Produk</h1>
            <p>Jelajahi seluruh produk di toko kami</p>
        </div>
    </div>

    <div class="category-filter">
        <a href="{{ route('shop.all') }}" class="category-chip active">Semua</a>
        @foreach($categories as $category)
            <a href="{{ route('shop.category', $category->slug) }}" class="category-chip">{{ $category->name }}</a>
        @endforeach
    </div>
    ...
    @else
        <div class="empty-state">
            ...
            <div>
                ...
                <p class="empty-state__text">Belum ada produk tersedia saat ini</p>
            </div>
        </div>
    @endif
</div>
